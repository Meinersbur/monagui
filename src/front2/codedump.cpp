...
VarCodeList::dump()
{
  ...
  for (VarCodeList::iterator i = begin(); i != end(); i++) {
    ...
    cout << "(" << std::hex << (unsigned long) (*i).code << std::dec << "," ;
    if ((*i).vars)
      (*i).vars->dumplist();
    cout << ")";
  }
...
Code::viz()
{
  if (!mark) {
    ...
    cout << " L" << std::hex << (unsigned long) this << std::dec << " [label=\"";
    dump(false);
    // printf(" {%d}", refs);
    cout << "\"];\n";
    mark = true;
...
Code_c::viz()
{
  if (!mark) {
    ...
    cout << " L" << std::hex << (unsigned long) this << " -> L" << (unsigned long) vc.code << std::dec << " [label=\"";
    vc.dumpsubst();
    cout << "\"];\n";
    mark = true;
  }
...
Code_cc::viz()
{
  if (!mark) {
    ...
    cout << " L" << std::hex << (unsigned long) this << " -> L" << (unsigned long) vc1.code << std::dec << " [label=\"";
    vc1.dumpsubst();
    cout << "\"];\n";
    //printf(" L%lx -> L%lx [label=\"", (unsigned long) this, (unsigned long) vc2.code);
    cout << " L" << std::hex << (unsigned long) this << " -> L" << (unsigned long) vc2.code << std::dec << " [label=\"";
    vc2.dumpsubst();
    cout << "\"];\n";
    mark = true;
  }
