...
#define YYLLOC_DEFAULT(Current, Rhs, N)    \
    if (Rhs == &yylsa[0]) {                    \
          (Current).first_offset = 0;      \
          (Current).first_line   = 1;        \
          (Current).first_column = 1;      \
          (Current).last_offset  = 0;       \
          (Current).last_line    = 1;         \
          (Current).last_column  = 1;       \
          (Current).filename     = "";          \
    } else {                                \
        yySetLocation(Current, Rhs, N);      \
    }

void yySetLocation(YYLTYPE &Current, YYLTYPE *Rhs, int N) {
  if (N > 0) {
      ...
      (Current).first_offset = (Rhs)[0].last_offset;
      (Current).first_line   = (Rhs)[0].last_line;
      (Current).first_column = (Rhs)[0].last_column;
      (Current).last_offset  = (Current).first_offset;
...
#define POS(p) Pos(p.filename, p.first_offset, p.first_line, p.first_column, p.last_offset, p.last_line, p.last_column)
...
static void yyerror(const char *s) {
    errorExit = true;
    yacc_yyerror(s, yylloc.filename, yylloc.first_offset, yylloc.first_line, yylloc.first_column, yylloc.last_offset, yylloc.last_line, yylloc.last_column);
}
...
bool g_yyHookCompletion = false;
YYLTYPE g_yyCompletionLoc;
int g_yyCompletionState = -1;
std::vector<int> g_yyCompletionStack;

YYLTYPE g_yyPrevDeclaration;
YYLTYPE g_yyNextDeclaration;
bool g_yyAfterCompletion;


static void yyUpdateCompletionState(int yyState, short int *yybottom, short int *yytop) {
    if (!g_yyHookCompletion)
        return;
    if (yylloc.filename == NULL)
        return;

    if (g_yyAfterCompletion) {
        if (g_yyNextDeclaration.filename != NULL)
            return;

        std::vector<int> stack;
        for (; yybottom <= yytop; yybottom++) {
            int yybot = *yybottom;
            stack.push_back(yybot);
        }

        if (isActiveToken(stack, findYytnameIndex("declaration"))) {
            g_yyNextDeclaration = yylloc;
        }
    } else {
        g_yyCompletionState = yyState;
        g_yyCompletionStack.clear();
        for (; yybottom <= yytop; yybottom++) {
            int yybot = *yybottom;
            g_yyCompletionStack.push_back(yybot);
        }

        if (isActiveToken(g_yyCompletionStack, findYytnameIndex("declaration")))   {
            g_yyPrevDeclaration = yylloc;
        }

#ifdef DEBUG
        fprintf(stderr, "%s == %s\n",yylloc.filename, g_yyCompletionLoc.filename);
#endif
        if (strcmp(yylloc.filename, g_yyCompletionLoc.filename) == 0) {
#ifdef DEBUG
          fprintf(stderr, "%d > %d\n",yylloc.first_offset, g_yyCompletionLoc.first_offset);
#endif
            if (yylloc.first_offset >= g_yyCompletionLoc.first_offset) {
#ifdef DEBUG
                fprintf(stderr, "Match yyState=%d  stacktop=%d, stacksize=%d offset=%d (Line %d Column %d)\n",yyState, g_yyCompletionStack.front(), (int)g_yyCompletionStack.size(), yylloc.last_offset, yylloc.last_line, yylloc.last_column);
                fprintf(stderr, "Previous declaration match: Offset %d (Line %d Column %d)\n", yylloc.last_offset, yylloc.last_line, yylloc.last_column);
#endif
                g_yyAfterCompletion = true;
            }
        }
    }
}
...
#define yylex() \
    yylex_real(); yyUpdateCompletionState(yystate, yyss, yyssp)
...
{
       0,   262,   262,   266,   268,   270,   272,   275,   278,   280,
     285,   287,   289,   292,   294,   296,   298,   300,   302,   304,
     306,   308,   311,   314,   316,   319,   322,   324,   326,   328,
     ...
     452,   454,   456,   460,   462,   464,   466,   468,   470,   472,
     474,   478,   481,   484,   487,   490,   493,   496,   499,   502,
     505,   510,   513,   518,   522,   525,   528,   531,   534,   537,
     542,   545,   548,   551,   556,   559,   562,   566,   569,   574,
     577,   582,   585,   590,   594,   597,   602,   605,   607,   611,
     614,   617,   621,   624,   627,   630,   633,   636,   639,   644,
     647,   652,   654,   656,   660,   662,   667,   670
};
#endif
...
yyreduce:
  ...
  case 53:
    ...
    {check_bits((yyvsp[(3) - (3)].string)); (yyval.untypedExp) = new UntypedExp_Dot((yyvsp[(1) - (3)].untypedExp), (yyvsp[(3) - (3)].string), POS((yylsp[(2) - (3)])));}
    break;
  ...
  case 67:
    ...
    {if (!(yyvsp[(5) - (6)].nameList)) yyerror("parse error"); (yyval.untypedExp) = new UntypedExp_RootPred((yyvsp[(3) - (6)].untypedExp), (yyvsp[(5) - (6)].nameList), POS((yylsp[(1) - (6)])));}
    break;
  ...
  case 97:
    ...
    {(yyval.arithExp) = new ArithExp_Subtr(new ArithExp_Integer(0, POS((yylsp[(1) - (2)]))), (yyvsp[(2) - (2)].arithExp), POS((yylsp[(2) - (2)])));}
    break;
