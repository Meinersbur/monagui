...
SymbolTable::Entry SymbolTable::dummyEntry;
		       ...
		       IdentList *univs, bool local, bool implicit)
{
  if (local)
    ...
    check(name);
  return insert(new VarEntry(name->str, type, noIdents, univs, implicit, local));
}
...
SymbolTable::dump()
{
  ...
  for (id = 0; id < (signed) noIdents; id++) {
    ...
    cout << " @";
    cout << std::left << std::setw(5) << offsets.off(id);
    cout << "  ";
    cout << std::left << std::setw(20) << e->string;
    cout << " ";
    cout << std::left << std::setw(12) << type;
...
MonaTypeTag SymbolTable::getType(Ident i) {
    Entry *e = identMap.get(i);
    return e->monaTypeTag;
}

char *SymbolTable::getName(Ident i) {
    Entry *e = identMap.get(i);
    return e->string;
}

bool SymbolTable::isLocal(Ident i) {
    Entry *e = identMap.get(i);
    VarEntry *v = dynamic_cast<VarEntry*>(e);
    if (v) {
        return v->local;
    }
    return false;
...
localStack(that.localStack), identMap(that.identMap)
{

  this->symbols = that.symbols;
  this->declarationTable = that.declarationTable;
