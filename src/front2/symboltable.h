#ifndef HEADER_F672DA279D8D6A5E
#define HEADER_F672DA279D8D6A5E
...
class SymbolTable {
  ...
  public:
    Entry():
        string(NULL), monaTypeTag((MonaTypeTag) -1), ident(-1) {} // dummy
  ...
  public:
    VarEntry(char *s, MonaTypeTag t, Ident i, IdentList *u, bool f, bool local) :
      Entry(s, t, i), universes(u), restriction(NULL), implicit(f), local(local) {}
    ~VarEntry()
...
public:
   ...
   //SymbolTable(const SymbolTable &from); // Copy ctor
  ~SymbolTable();

  char  *insertString(char *);

  Ident  insertPred(Name*);
  Ident  insertVar(Name*, MonaTypeTag, IdentList *univs, bool local = false, bool implicit = false);
  ...
  MonaTypeTag getType(Ident i);
  char *getName(Ident i);
  bool isLocal(Ident i);
